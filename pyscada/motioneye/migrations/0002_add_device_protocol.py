        ...
        DeviceProtocol.objects.using(db_alias).bulk_create(
            [
                DeviceProtocol(
                    pk=PROTOCOL_ID,
                    protocol="motioneye",
                    description="MotionEye Interface",
                    app_name="pyscada.motioneye",
                    device_class="pyscada.motioneye.device",
                    daq_daemon=True,
                    single_thread=True,
                ),
            ]
...
def reverse_func(apps, schema_editor):
    ...
    DeviceProtocol = apps.get_model("pyscada", "DeviceProtocol")
    db_alias = schema_editor.connection.alias
    DeviceProtocol.objects.using(db_alias).filter(protocol="motioneye").delete()


class Migration(migrations.Migration):
    dependencies = [
        ("motioneye", "0001_initial"),
    ]
