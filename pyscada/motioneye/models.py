...
class MotionEyeDevice(models.Model):
    motioneye_device = models.OneToOneField(
        Device, null=True, blank=True, on_delete=models.CASCADE
    )
    motioneye_server = models.ForeignKey(
        MotionEyeServer, null=True, blank=True, on_delete=models.CASCADE
    )
    camera_id = models.PositiveSmallIntegerField(
...
class MotionEyeVariable(models.Model):
    motioneye_variable = models.OneToOneField(
        Variable, null=True, blank=True, on_delete=models.CASCADE
    )
    service_actions_choices = (
        ("snapshot", "Snapshot"),
        ("lock", "Lock"),
        ("unlock", "Unlock"),
        ("light_on", "Light on"),
    ...
    # increment this list
    service_not_boolean = {
        "boolean": {0: ["False", "False"], 1: ["True", "True"]},
        "(left|right)_text": {
        ...
        "recording_mode": {
            6: ["motion-triggered", "Motion triggered"],
    ...
    def save(self, *args, **kwargs):
        ...
        for field in self.service_not_boolean:
            if re.compile(field, re.IGNORECASE).match(self.service):
                ...
                d, created = Dictionary.objects.get_or_create(
                    name="MotionEye_non_boolean_services"
                )
    ...
    def validate_unique(self, exclude=None):
        qs = MotionEyeVariable.objects.filter(
